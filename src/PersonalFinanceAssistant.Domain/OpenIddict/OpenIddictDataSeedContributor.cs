...
namespace PersonalFinanceAssistant.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("PersonalFinanceAssistant") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "PersonalFinanceAssistant", DisplayName = "PersonalFinanceAssistant API", Resources = { "PersonalFinanceAssistant" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["PersonalFinanceAssistant_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["PersonalFinanceAssistant_Web:RootUrl"]!.EnsureEndsWith('/');

            /* PersonalFinanceAssistant_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["PersonalFinanceAssistant_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["PersonalFinanceAssistant_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["PersonalFinanceAssistant_Swagger:RootUrl"]?.TrimEnd('/');
